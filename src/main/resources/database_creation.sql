...
VALUES	('CUSTOMER_NAME1', 'CUSTOMER_ADDRESS1', 12.1),
        ...
        ('CUSTOMER_NAME6', 'CUSTOMER_ADDRESS5', 12.5);
...
VALUES	('BOOK_NAME1', 'BOOK_ADDRESS1', 647.1, 9),
		('BOOK_NAME1', 'BOOK_ADDRESS2', 654.1, 9),
		('BOOK_NAME2', 'BOOK_ADDRESS3', 844.1, 8),
		('BOOK_NAME3', 'BOOK_ADDRESS4', 561.3, 7),
		('BOOK_NAME4', 'BOOK_ADDRESS5', 989.4, 6),
        ('WINDOWS for dummies', 'BOOK_ADDRESS6', 67.4, 6),
        ('BOOK_NAME7', 'BOOK_ADDRESS7', 3000, 6);
...
select * from orders;
        ...
        SELECT NAME, COST FROM BOOK GROUP BY NAME, COST;
        
        SELECT * FROM BOOK GROUP BY NAME, COUNT;
        
        select * from customer;
        
        select * from customer group by address;
...
SELECT DISTINCT NAME, COUNT FROM BOOK;

SELECT DISTINCT ADDRESS FROM CUSTOMER;

SELECT * FROM ORDERS GROUP BY MONTH(ORDER_DATE);
...
SELECT C.NAME, S.NAME FROM ORDERS O 
JOIN CUSTOMER C ON C.ID = O.CUSTOMER_ID
JOIN STORE S ON S.ID = O.STORE_ID;

SELECT O.ORDER_DATE, C.NAME, C.DISCOUNT * B.COST /100, B.NAME, O.COUNT FROM ORDERS O
JOIN CUSTOMER C ON C.ID = O.CUSTOMER_ID
